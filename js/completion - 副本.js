...
$(document).ready(function() {
	...
	console.log(diffIllMapping != null)
	//初始化曲名映射
	diffSongNameMapping = getTitleMapping();
	//初始化曲绘映射
	diffIllMapping = getImageMapping();
	console.log(diffSongNameMapping)
	console.log(diffIllMapping)
	...
	$.ajax({
		...
		success: function(resp) {
			...
			for (const song in constant) {
				// let t = new PlayResult('', row.split('-')[0], row.split('-')[1], '-1',
				// 	0, 0, 0, 0, c,
				// 	0, index);
				...
				// completionTable[[constant[song]['Past']]][song-constant] = 1;
				...
				// completionTable[[constant[song]['Present']]][song-constant] = 1;
				...
				// completionTable[[constant[song]['Future']]][song-constant] = 1;
				...
				// completionTable[[constant[song]['Beyond']]][song-constant] = 1;
...
function refillCurrentArray(array = currentArray) {
	...
	for (i; i < ary.length; i++) {
		for (j; j < c.length; j++) {
			if (ary[i].songId == c[j].songId && ary[i].difficulty == c[j].difficulty) {
				c[j] = ary[i];
				break;
			}
		}
	...
	constant = [];
	constant = c;
	generateUnits(constant, constant.length);
...
function changeBound() {
	...
	refillCurrentArray(currentArray, constant)
}
