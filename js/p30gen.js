...
function displayB30Value(data, flag) {
	...
	b30PTTContainer.textContent = "P30 :";
...
function transferP30(data) {
  // 将CSV字符串转换为二维数组（假设逗号是分隔符）
  var rows = data.split('\n');
  var header = rows[0]; // 保存第一行（表头）
  var csvArray = rows.slice(1).map(function(row) {
    return row.split(',');
  });

  // 过滤出第7和第8列（按0索引为第6和第7列）均为0的行
  var filteredArray = csvArray.filter(function(item) {
    return (Number(item[6]) === 0 && Number(item[7]) === 0);
  });

  // 将过滤后的数组转回CSV格式，并在开头加入表头
  var result = header + '\n' + filteredArray.map(function(item) {
    return item.join(',');
  // }).join('\n') + '\n'; // 添加最后一行的换行符以保持原始CSV格式
  }).join('\n'); // 添加最后一行的换行符以保持原始CSV格式
