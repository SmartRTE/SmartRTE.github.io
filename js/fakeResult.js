...
let difficultyTextShadowPattern =
	'0 0 3.5px COLOR, 0 3.5px 2px COLOR, 2px 2px 2px COLOR, 3.5px 0 2px COLOR, 2px -2px 2px COLOR, 0 -3.5px 2px COLOR, -2px -2px 2px COLOR, -3.5px 0 2px COLOR, -2px 2px 2px COLOR, 3px 3px COLOR, -3px 3px COLOR, 3px -3px COLOR, -3px -3px COLOR'
...
function applySettings(values) {
	...
	$('#constant').css('text-shadow', difficultyTextShadowPattern.replaceAll('COLOR', difficultyShadowColor[values[
		'difficulty']]));
	$('#plus-mark').css('text-shadow', difficultyTextShadowPattern.replaceAll('COLOR', difficultyShadowColor[values[
		'difficulty']]));
...
function adjustPotentialPosition(hashFlag) {
	if (!hashFlag) {
		...
		$('#avatar-border').attr("src", 'img/topbar/char_icon_border.png').removeClass('character-border-hash')
			.addClass('character-border');
		$('#potential-change').css("left", "50%");
	} else {
		$('#hash-mark').css("display", 'block');
		$('#hash-number').css("display", 'block');
		$("#fragment-text").css("display", 'none');
		$('#avatar-border').attr("src", 'img/topbar/usercell_shape_bg.png').removeClass('character-border').addClass(
			'character-border-hash');
...
async function readSongList() {
	try {
		...
		songlist = slst.songs;
		songlist.splice(127, 1);
		songlist.sort((a, b) => {
			let titleA = a.title_localized['en'].toUpperCase(); // ignore upper and lowercase
			let titleB = b.title_localized['en'].toUpperCase(); // ignore upper and lowercase
			if (titleA < titleB) {
				return -1;
			}
			if (titleA > titleB) {
...
async function generateSongListSelection(songlist) {
	...
	songlist.forEach(function(song, index) {
		if (index != 127) {
			// console.log(song);
			let opt = $(`<li id="${song.id}" onclick="applyQuickSelection(songlist, ${index})">`).append($(
				'<img>').attr('src', `Illustration/${song.id}.jpg`)).append($('<span>').text(song.title_localized['en']));
			sls.append(opt);
		}
	});
...
function handleCourseBanner() {
	let baseWidth = parseFloat($('#username').css('width').replace("px", ''));
	let textWidth = parseFloat($('#username-text').css('width').replace("px", ''));
	let standardWidth = 225;
	let offset = (baseWidth - textWidth) / 2 - standardWidth;
	console.log(baseWidth, textWidth)
	console.log(offset)
	$('#course-banner').css('left', offset + 'px')
}